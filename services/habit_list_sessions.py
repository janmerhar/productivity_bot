...
_COLLECTION = mongo_db["habit_list_sessions"]
...
def _ensure_indexes() -> None:
    _COLLECTION.create_index(
        [("session_id", 1)],
        unique=True,
        name="habit_list_sessions_session_id",
    )
    _COLLECTION.create_index(
        [("expires_at", 1)],
        expireAfterSeconds=0,
        name="habit_list_sessions_expires_at_ttl",
    )
...
def create_session(state: Dict[str, Any]) -> str:
    ...
    for _ in range(5):
        ...
        try:
            ...
            continue
        return session_id
    raise RuntimeError("Could not create a habit list session.")
